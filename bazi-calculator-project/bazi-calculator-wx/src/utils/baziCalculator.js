...
export { DI_ZHI_YIN_YANG, DI_ZHI_CANG_GAN };
...
function analyzeShiShen(bazi, wuxingInfo) {
  ...
  const result = {
    ...
    day: determineShiShen(dayGan, bazi.day[0]),
...
// 判断五行生克关系：A是否生B
export function isGenerating(wuxingA, wuxingB) {
  const generating = {
    '木': '火',
    '火': '土',
    '土': '金',
...
// 判断五行生克关系：A是否克B
export function isRestricting(wuxingA, wuxingB) {
  const restricting = {
    '木': '土',
    '土': '水',
    '水': '火',
