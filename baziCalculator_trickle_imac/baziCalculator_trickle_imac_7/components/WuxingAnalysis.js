      ...
      <div className="analysis-item slide-in" data-name="wuxing-analysis">
        ...
        <div className="analysis-content">
          ...
          <div className="mt-4" data-name="wuxing-analysis-description">
            <h4 className="font-semibold mb-2">五行分析解释：</h4>
                ...
                if (strength[wx] === 'strong') {
                  return (
                    <li key={wx} data-name={`wuxing-${wx}-description`}>
                      ...
                      {' 在命局中较为旺盛，数量 '}{count[wx].toFixed(1)}{', 占比 '}{percentages[wx]}{'%。'}
                      {getWuxingDescription(wx, 'strong')}
                    </li>
                  );
                  ...
                  return (
                    <li key={wx} data-name={`wuxing-${wx}-description`}>
                      <span className={`font-medium wuxing-${getWuxingClass(wx)}`}>{wx}</span>
                      {' 在命局中较为薄弱，数量 '}{count[wx].toFixed(1)}{', 占比 '}{percentages[wx]}{'%。'}
                      {getWuxingDescription(wx, 'weak')}
                    </li>
                  );
                }
                return (
                  <li key={wx} data-name={`wuxing-${wx}-description`}>
                    <span className={`font-medium wuxing-${getWuxingClass(wx)}`}>{wx}</span>
                    {' 在命局中平衡，数量 '}{count[wx].toFixed(1)}{', 占比 '}{percentages[wx]}{'%。'}
                  </li>
                );
              })}
            </ul>
...
function getWuxingBarColor(wx) {
  switch (wx) {
    ...
    case '土': return 'yellow-500';
    case '金': return 'gray-400';
